# rollout_updated.py — modified from rollout.py with these changes:
#   1. Cube spawns uniformly random within the training bounding box
#      (x: 0.23~0.48, y: -0.63~-0.44) instead of sampling fixed JSON positions.
#   2. Early termination if the cube is not captured within --capture_timeout_s
#      seconds (default 15 s). Simulates "one miss = stop".
#   3. Cube sliding into basket without being lifted (captured=False) does not count as success.
#   4. Controller gracefully handles headless mode (no appwindow).
#   5. env.reset() wrapped in torch.no_grad() to avoid InferenceMode inplace update error.
...
parser = argparse.ArgumentParser(
    description="Synchronous LeRobot inference — random spawn + capture timeout."
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Number of evaluation rounds. 0 = run forever.",
)
parser.add_argument(
    "--policy_type",
    type=str,
    default="lerobot-smolvla",
    help="Local LeRobot policy type, e.g. lerobot-diffusion.",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=float,
    default=None,
    help="Override cube_linear_speed_range with a fixed speed (m/s).",
)
parser.add_argument(
    "--capture_timeout_s",
    type=float,
    default=15.0,
    help="Stop episode if cube not captured within this many seconds (default 15).",
)
parser.add_argument(
    "--debug_policy_shapes",
    action="store_true",
    help="Print observation and action tensor shapes.",
)
...
class LeRobotSyncPolicy:
    ...
    def _build_raw_observation(self, observation_dict: dict) -> dict[str, Any]:
        ...
        if self.task_type == "so101leader":
            ...
            raise ValueError(
                f"Task type {self.task_type} not supported."
            )
    ...
    def _convert_actions_to_leisaac(self, action_tensor: torch.Tensor) -> np.ndarray:
        ...
        if actions.shape[-1] != self.action_dim:
            raise ValueError(
                f"Expected {self.action_dim} action values, got {actions.shape[-1]}."
            )
        return actions
...
def get_policy_type(policy_type_arg: str) -> str:
    if not policy_type_arg.startswith("lerobot-"):
        raise ValueError(
            f"Only local LeRobot policies supported, got '{policy_type_arg}'."
        )
...
def main():
    ...
    # Use workspace bounds matching the training bounding box so the env
    # falls back to uniform random spawn instead of fixed JSON positions.
    # JSON positions cover x: 0.23~0.48, y: -0.63~-0.44.
    # The fallback sampler adds ±0.08 margin, so set workspace slightly wider.
    if hasattr(env_cfg, "cube_workspace_x"):
        env_cfg.cube_workspace_x = (0.15, 0.56)
        env_cfg.cube_workspace_y = (-0.71, -0.36)
        print("[rollout] cube workspace set to training bounding box (random spawn mode)")

    env: ManagerBasedRLEnv = gym.make(task_id, cfg=env_cfg).unwrapped

    # Clear JSON positions so the env uses uniform random spawn within the
    # workspace bounds set above instead of the fixed training positions.
    if hasattr(env, "_json_positions"):
        env._json_positions = []
        print("[rollout] _json_positions cleared → uniform random spawn enabled")
    ...
    capture_timeout_steps = int(args_cli.capture_timeout_s * args_cli.step_hz)
    print(
        f"[rollout] capture_timeout = {args_cli.capture_timeout_s}s "
        f"({capture_timeout_steps} steps)",
        flush=True,
    )
    ...
    while max_episode_count <= 0 or episode_count <= max_episode_count:
        ...
        success, time_out, captured, capture_timed_out = False, False, False, False
        step_count = 0
        ...
        while simulation_app.is_running():
            with torch.no_grad():
                ...
                for action_index in range(
                    ...
                    if not captured and _cube_is_captured():
                        captured = True
                        print(
                            f"[rollout] cube captured at step {step_count} "
                            f"({step_count / args_cli.step_hz:.1f}s)",
                            flush=True,
                        )

                    # Early termination: cube not captured within capture_timeout
                    if not captured and step_count >= capture_timeout_steps:
                        capture_timed_out = True
                        print(
                            f"[rollout] capture timeout ({args_cli.capture_timeout_s}s) — "
                            "cube not captured, ending episode early.",
                            flush=True,
                        )
                        break
            ...
            if success and captured:
                ...
                with torch.no_grad():
                    obs_dict, _ = env.reset()
                break
            elif success and not captured:
                print(f"[Evaluation] Episode {episode_count} cube slid into basket (not captured, not counted)")
                episode_count += 1
                policy.reset()
                with torch.no_grad():
                    obs_dict, _ = env.reset()
            ...
            if capture_timed_out:
                print(f"[Evaluation] Episode {episode_count} capture timeout! (no grasp in {args_cli.capture_timeout_s}s)")
                episode_count += 1
                policy.reset()
                with torch.no_grad():
                    obs_dict, _ = env.reset()
